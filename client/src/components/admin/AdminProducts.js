...
const emptyForm = {
  name: '', description: '', price: '', category: 'signature-latte', tags: '', available: true, featured: false
};
    ...
    reader.onload = async (evt) => {
      try {
        ...
        for (const line of dataLines) {
          ...
          formData.append('category', category || 'signature-latte');
        ...
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-group">
                <label>Description {form.category === 'milk-option' && '(Optional)'}</label>
                <textarea 
                  value={form.description} 
                  onChange={e => setForm({ ...form, description: e.target.value })} 
                  required={form.category !== 'milk-option'} 
                />
              </div>
              <div className="admin-form" style={{ padding: 0, boxShadow: 'none' }}>
                <div className="form-row">
                  <div className="form-group">
                    <label>Price ($) {form.category === 'milk-option' && '(Optional)'}</label>
                    <input 
                      type="number" 
                      step="0.01" 
                      min="0" 
